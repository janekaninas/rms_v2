...
>
> **Revision note (v0.9 — corrects Monthly Performance, which never adopted §7a):** §7a's engine was correctly wired into All Bookings and Road to Target when each was built, but Monthly Performance's revenue cell (and therefore Summary's rollup, which reads the same data) was left summing only actual `daily_revenue` rows — reproducing exactly the "blank until Room Revenue Breakdown exists" behavior §7a was written to eliminate. **New §7a-D states explicitly**: this section's allocation is the one and only per-night revenue calculation, used by every report without exception. An occupied night with a valid booking total is never blank in any report; it is Actual once a Room Revenue Breakdown row exists and the §7a-A/B estimate otherwise. See `REPORTING_LOGIC.md` §2a for the corrected Monthly Performance behavior and the enhanced occupancy drill-down (§2c).
...
### D. `[CORRECTED — v0.9]` One engine, every report — including Monthly Performance and Summary

This section's allocation logic is not an All-Bookings-specific or Road-to-Target-specific concept — it is *the* per-night revenue figure for a reservation, period, and every report that shows revenue for an occupied night reads it from here. Concretely: **an occupied night with a valid booking total is never blank or zero in any report** — Monthly Performance's per-date revenue cell and Summary's monthly rollup (`REPORTING_LOGIC.md` §2a/§3) apply this section exactly as All Bookings and Road to Target already do, showing the Actual figure where a Room Revenue Breakdown row exists and the progressive/override Estimated figure otherwise. An earlier revision of `REPORTING_LOGIC.md` §2a described Monthly Performance's revenue cell as actual-only ("blank until Room Revenue Breakdown exists," mirroring the legacy workbook's `IF(date<=today, computed, blank)`) — that instruction is superseded by this section and is no longer correct; the blank-until-actual behavior was never a deliberate business rule, it was this section's confirmed model not yet having been carried into Monthly Performance's implementation. A cell shows **blank only when nothing occupies it** (zero occupancy) and **"Incomplete" only when the occupying reservation itself has no booking total or no resolvable `channel_payment_rules` row** (`MISSING_PAYMENT_RULE`) — never as a stand-in for "Room Revenue Breakdown hasn't been uploaded yet."
...
19. **`[PARTIALLY RESOLVED — this revision]`** Airbnb, Booking.com, Expedia settlement file formats. Confirmed as the Day-7 priority order (§6). Real sample exports for **Airbnb and Booking.com** have now been supplied and confirmed against the settlement-import framework (`IMPORT_LOGIC.md` §8, `DATA_MODEL.md` §7): Booking.com is a flat file (one `Reservation` row per settlement line, rows sharing a `Payout ID`/`Payout date` group into one batch, its own `Amount` column is explicitly *not* the settlement figure — `Net` is); Airbnb interleaves `Type = Payout` batch-summary rows with `Type = Reservation` detail rows, each attributed to the nearest preceding Payout row, with the Payout row's own `Paid out` value as the batch's authoritative net settlement amount (validated, not overridden, by the sum of its reservation lines — confirmed to reconcile exactly across every batch in the supplied sample). Both channels' real column names carry additional fields worth preserving for drill-down (Booking.com: Commission, VAT for online platform services, Payments Service Fee, Payment status, Reservation status; Airbnb: Service fee, Gross earnings, Booking date, Start/End date, Nights, Listing) — captured via a generic per-channel "preserve extra fields" mapping (`extra_fields` jsonb on `ota_settlement_lines`), never a hardcoded per-OTA column. **Expedia's real format remains unsupplied and still needs a sample file before its mapping can be finalized** — do not guess its column layout from the worked examples in §6 alone.
