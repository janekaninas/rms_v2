...
| guest_name | text | |
| booking_date | date, nullable | can be `#N/A`/absent in VHP exports; not a hard requirement |
...
| commission | numeric | computed; `0` (not merely absent) when `source_amount_basis = NET_AFTER_OTA_DEDUCTIONS`, since there's nothing left to deduct |
| commission_vat | numeric | computed ("VAT on OTA Commercial Deductions") |
| pb1 | numeric | computed |
| net_revenue | numeric | derived: `commercial_revenue_basis_amount − commission − commission_vat − pb1` |
| source_import_id | fk → imports.id | |
| created_at, updated_at | | |
...
### Future booked revenue and estimated daily allocation `[NEW]`
