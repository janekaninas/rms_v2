...
### `[NEW]` Villa lifecycle controls: deactivate/reactivate/remove

Beyond Add/Edit, the Villas page offers:

- **Deactivate / Reactivate** — a one-click toggle on `villas.active`. Per §11, this only ever affects current-config surfaces (dropdowns, this page's own listing); it never touches `villa_id` resolution on any reservation, never removes a row from any historical report, and needs no recomputation.
- **Remove**, but **only when safe** (CLAUDE.md rule 15: never delete reservation/cancellation history). Before deleting, the app counts every `RESTRICT`-constrained reference — `reservations`, `daily_revenue`, `room_villa_mapping`, `channel_payment_rules`, `revenue_targets` — all by `villa_id`. If any count is non-zero, the delete is refused with a specific message naming what's referencing it (e.g. "referenced by 4 reservation(s), 4 daily revenue row(s), 3 room mapping(s)") and the villa is left untouched; Jane deactivates it instead. A villa with zero references (created by mistake, never used) deletes outright. No schema change — this is an application-level pre-check in front of the existing `RESTRICT` foreign keys.
...
**`[NEW]` Mapping CRUD and automatic re-resolution.** Configuration → Villa Mapping supports Add/Edit/Remove on this table directly (`portfolio`, `match_type`, `raw_value`, `villa_id`, `priority`), plus a duplicate/conflict guard: `(portfolio, match_type, raw_value)` is DB-unique, and a violation surfaces as "A mapping for this portfolio + match type + raw value already exists. Edit that mapping instead of adding a duplicate." rather than a raw constraint error. An "Unresolved" banner lists every open `UNKNOWN_VILLA` exception's raw room number/type grouped with an affected-reservation count, each with a one-click "Map it" action that opens Add Mapping pre-filled with that raw value.

Every create/edit/delete on this table immediately **re-resolves every already-imported reservation from its already-stored data** — `reservations.room_number`/`room_type` are the same persisted raw values `resolveVilla()` matched against at import time (`IMPORT_LOGIC.md` §6), so a mapping change can replay that exact matching logic against every existing reservation without re-uploading Arrival Report/Bookings/Cancellations/Room Revenue. For every reservation whose resolved `villa_id` changes as a result: the resolved reference is updated (bulk, grouped by target villa — not one `UPDATE` per row); any newly-unresolved reservation opens an `UNKNOWN_VILLA` exception and any newly-resolved one closes its open exception (deduped, same open/resolve bookkeeping as import-time); and the reservation is passed through the same `recomputeReservations()` engine used after imports and override approvals, so its nightly allocation, Expected Settlement, and downstream Monthly Performance/Summary/Road to Target figures refresh automatically. The raw `room_number`/`room_type` values themselves are never altered — only the resolved `villa_id` reference — preserving the original imported values for auditability (CLAUDE.md rule 15). No schema change was required for this: the raw values needed for re-matching were already persisted on `reservations`.
