...
>
> **Revision note (v0.11 — MISSING_PAYMENT_RULE channel clarity + fix link):** §2a and §5 now require every `MISSING_PAYMENT_RULE` surface to name the affected channel and link directly to Configuration → Channel Payment Rules with it preselected. Implementing this surfaced a real conflation bug in Monthly Performance's "Incomplete" cell (present since v0.9): it was treating "no booking total at all" the same as a genuine unresolved payment rule, when the allocation engine only actually returns a null (excluded) `netRevenue` for the latter — a no-total-but-resolved-rule night computes a real, if degenerate, figure and was never truly "Incomplete" by the engine's own definition. §2a is corrected accordingly; Summary is unaffected since it reads the same corrected data.
>
> **Revision note (v0.12 — footer fix, Summary month range, Villa/Mapping management, implemented):** §2f is corrected: the Monthly Performance rollup footer is confirmed **non-sticky** (`DESIGN_SYSTEM.md` §3b) after the sticky-bottom approach proved to visually overlap the last daily rows during scroll — a plain in-flow footer with a sticky-left label column achieves the same alignment requirement without that failure mode. §3 (Summary) is corrected: the month range is now January of the current year through 12 months ahead of the current month, recomputed dynamically (not "current month through year-end"), and this section states explicitly, by construction, that Summary shares Monthly Performance's exact revenue calculation with no divergent path. This revision also documents (in `DATA_MODEL.md` §1/§2 and `IMPORT_LOGIC.md` §6) that Villa and Room/Villa Mapping configuration now have full CRUD, and that a mapping change automatically re-resolves already-imported reservations from already-stored data — no calculation-logic change to any report, and no new report.
>
> **Revision note (v0.13 — Day 5, Settlement Reconciliation built):** §10 is implemented as specified, with the Expected/Settled/Variance/Status figures always computed **live** from current source data at page-render time (not read from a cached column) — a reservation's `expected_settlement_amount` can change after its settlement batch was committed (e.g. a `MISSING_PAYMENT_RULE` getting configured later), and the page must reflect that immediately, not after a separate refresh step. `NEEDS_REVIEW` is used both for an ambiguous multi-candidate line match and for a batch whose allocated reservation(s) include an open `MISSING_PAYMENT_RULE` — the variance cell reads `MISSING_PAYMENT_RULE` rather than a misleading number in the latter case, extending `FINANCIAL_LOGIC.md` §5's no-silent-fallback rule to settlement reconciliation. Drill-down (batch → lines → reservation) reuses the exact same `ReservationDrilldown` component and nightly-allocation computation as All Bookings — not a rebuilt one.
...
- **`[CORRECTED — v0.11]` "Incomplete" and "no booking total" are two different, unrelated problems — never conflated into one label.** "Incomplete" means a genuine `MISSING_PAYMENT_RULE` (the channel — or, rarer, the villa mapping — has no resolvable `channel_payment_rules` row); it is the one that is *never used as a stand-in for "no Room Revenue Breakdown yet"* (that case is Estimated, not Incomplete), and it is the one excluded from the villa's monthly rollup (§2b). A reservation with a resolvable channel/rule but simply no booking total imported yet computes a real (if degenerate) figure through the same engine and is not "Incomplete" at all — it may still be worth a distinct, non-blocking visual note (e.g. "No total"), but it must not be presented as a payment-rule problem, and it must not be excluded from the rollup the way a genuine `MISSING_PAYMENT_RULE` night is.
- **`[NEW — v0.11]` Every "Incomplete"/`MISSING_PAYMENT_RULE` surface must name the channel, not just say "Incomplete."** Wherever this exception is shown — a Monthly Performance cell's tooltip, the cell drill-down, a reservation's own drill-down banner, the Road to Target exclusion banner — the affected channel's display name must be visible (directly, or via hover), and a direct link to Configuration → Channel Payment Rules with that channel preselected (`?channel=<channel_id>`, opening the Add Rule dialog immediately) must be offered wherever the channel is known, so the fix is at most one click from the point the problem was noticed rather than a separate lookup-and-navigate step.
- A cell touched by more than one reservation (multi-unit villa) sums their individual night figures; if any one of them has a genuine `MISSING_PAYMENT_RULE`, the cell is Incomplete (excluded from the villa's monthly rollup, §2b) even if the others resolved fine — never blend a resolved and an unresolved figure into one silently-partial number.
...
The matrix behaves like a frozen-pane spreadsheet, not a page that happens to contain a wide table: the page's own title/view-toggle/month-selector stay fixed while only the matrix scrolls, the matrix scrolls independently in both directions (never the whole browser page), its villa-column headers and Date column stay pinned against that scrolling, and the app shell (sidebar) is unaffected regardless of how tall the matrix gets. The bottom rollup rows (Room Nights Sold / Occupancy % / ARR / Net Revenue) stay column-aligned during horizontal scroll and never overlap the daily rows above them, but are **not** bottom-sticky — `[REVISED, v0.12]` see `DESIGN_SYSTEM.md` §3b for why a non-sticky footer is the confirmed, adopted approach, not just a fallback. Full implementation spec — the app-shell fix, the bounded-container technique, sticky-cell conventions, and the z-index scale — is `DESIGN_SYSTEM.md` §3b; this section states the *requirement*, that doc states *how*.
...
Shows, **by month and by portfolio** (Aasha, Balinest, combined):
- Revenue (net)
- Occupancy %
- ARR
- Room Nights Sold

`[REVISED]` **Month range: January of the current calendar year through 12 months ahead of the current month, recomputed dynamically on every load — not "current month through year-end."** Worked example: today = September 2026 → the range is January 2026 through September 2027 (21 months). This is computed from `today` at request time (never a hardcoded year), so the range itself shifts forward as time passes — it is not a fixed window that needs manual updating. The prior "current month through December" range was wrong in two ways at once: it hid every already-elapsed month of the current year (reading as "missing" data for those months), and it produced a shrinking window as the year went on rather than a stable forward-looking horizon.

This is a monthly rollup of the same `daily_revenue` + `reservations` aggregations used in Monthly Performance, just summed across all villas in a portfolio per month rather than shown per-villa-per-date. It should be derivable from the same underlying queries/rollup logic — do not build a separate, divergent calculation path for Summary vs. Monthly Performance; both must agree by construction; if profiling later shows the Summary page needs a precomputed month-level rollup table, add that as a materialized rollup fed by the same source-of-truth query, not a re-implementation. **Confirmed by construction**: Summary calls the identical `loadMonthlyPerformanceData`/`rollupVilla`/`aggregateRollups` functions Monthly Performance uses, per month in its range, and aggregates across villas — there is no second revenue calculation anywhere in Summary. A month's Summary total must equal that month's Monthly Performance Net Revenue footer total for the same portfolio scope; §5's `MISSING_PAYMENT_RULE` exclusion rule applies identically in both.
...
**`[NEW — v0.11]`** Since the fix for `MISSING_PAYMENT_RULE` is always the same action (add or correct a row in Configuration → Channel Payment Rules for the specific channel involved), every surface showing this exception must name that channel and offer a direct link there with the channel preselected, per §2a's corresponding rule — this applies uniformly across All Bookings' row (Channel is already a visible pinned column there), its reservation drill-down banner, Monthly Performance's cell tooltip and drill-down, and the Road to Target exclusion banner.
