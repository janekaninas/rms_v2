...
- **`[CORRECTED — mid-Day-3 fix]`** The initial nightly-allocation implementation used one calculation path for every booking type; this was corrected to the two confirmed regimes — OTA (Booking/Arrival Report total is authoritative, Room Revenue Breakdown only clarifies the nightly split) vs. Direct/Individual/Travel-Agent (system data untrusted; an approved manual override is authoritative with an even nightly split) — per `FINANCIAL_LOGIC.md` §7a. Also corrected: Booking Date (`booking_date` = New Bookings' `Created Date`) is now a visible All Bookings column (`REPORTING_LOGIC.md` §1 always specified it; the initial build omitted it) and is protected from being overwritten with null by a later import that has no creation-date source (`IMPORT_LOGIC.md` §1/§11); All Bookings gained frozen Reservation #/Guest/Channel/Villa columns and the reservation drill-down gained a wider panel, a booking-summary header, and a Print action (`DESIGN_SYSTEM.md` §9a). All of this landed within Day 3 — no Day 4 scope was started to fix it.
- **`[NEW — platform-wide shell, landed alongside the above]`** The left sidebar (built Day 1) is now collapsible, per `DESIGN_SYSTEM.md` §3a/`PRODUCT_SPEC.md` §5 — this is navigation-shell UI applying to every page regardless of day, not a Day-3-specific or revenue-specific feature; recorded here only because it shipped in this same pass.
- Deploy and verify in production.
...
- **`[DONE]`** Monthly Performance, Summary, and Road to Target (core field list) all built and verified against real allocation-engine output. Notes on how each landed:
  - Monthly Performance's occupancy/revenue matrices reuse the same sticky-column technique as All Bookings (`DESIGN_SYSTEM.md` §9a) rather than AG Grid — evaluated per CLAUDE.md's stack note and rejected for this dataset size (~1,000 cells, no virtualization/inline-editing need); see `DESIGN_SYSTEM.md` §4's evaluation note. Cell drill-down is computed on demand (only the clicked villa+date), not prefetched for the whole matrix, per §2e.
  - A shared query module (`src/lib/reporting/monthly-rollup.ts`) backs both Monthly Performance's per-villa-per-date matrix and Summary's per-portfolio-per-month rollup, so the two can never diverge (`REPORTING_LOGIC.md` §3's explicit requirement).
  - Road to Target's Currently Booked Revenue reuses `allocateReservationNights()` — the exact same OTA-progressive/Direct-override-aware engine from Day 3 (`FINANCIAL_LOGIC.md` §7a) — for every not-yet-actual night, rather than a separate simpler estimate; a reservation with an open `MISSING_PAYMENT_RULE` exception has its nights excluded from the total, surfaced via a visible banner, not silently included or blocked entirely.
  - No portfolio `revenue_targets` row exists yet for any month past August 2026 (the only seeded row) — Road to Target correctly shows an explicit "no target configured" state with a link to Configuration → Revenue Targets rather than inventing or carrying one forward; add September onward there to see live Achievement/Gap figures.
  - Channel Performance **deliberately not built** — per its own Day-7-optional status, and Day 4 had room to do the three required reports well rather than stretch across four; still a same-day-4 candidate on a future pass, no code precludes it.
- Deploy and verify in production.
