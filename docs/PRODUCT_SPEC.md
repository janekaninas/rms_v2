...
>
> **Revision note (v0.9 — Monthly Performance revenue correction):** Day 4's initial Monthly Performance build summed only actual `daily_revenue` rows for its revenue cells, reproducing the legacy workbook's "blank until the day passes" behavior instead of the confirmed engine (`FINANCIAL_LOGIC.md` §7a/§7a-D) already used everywhere else. Corrected: an occupied night with a valid booking total always shows a figure (Actual or Estimated), and the cell drill-down now explains the villa/date result (occupied/available room nights, totals, actual/estimated composition, per-reservation detail linking to the full reservation record) rather than listing bare numbers. Full spec: `REPORTING_LOGIC.md` §2a/§2c. No new domain, page, or architecture change — a calculation-and-drill-down correction within the already-approved Day 4 scope.
>
> **Revision note (v0.10 — spreadsheet-style scrolling, UI robustness only):** Monthly Performance now behaves like a bounded spreadsheet: the app shell (sidebar + page header) stays fixed, and the matrix itself independently scrolls both directions with sticky column headers, a sticky Date column, and (where practical) a sticky rollup footer — full behavioral spec in `DESIGN_SYSTEM.md` §3b, which also fixes a shell-level bug (the sidebar could scroll away on any page taller than one viewport, not just Monthly Performance). No calculation, page, or navigation-structure change.
>
> **Revision note (v0.11 — corrections pass: footer, Villa/Mapping management, Summary range, implemented):** Monthly Performance's rollup footer is corrected from sticky-bottom (which visually overlapped the last daily rows during scroll) to a non-sticky footer that stays column-aligned — `DESIGN_SYSTEM.md` §3b. Configuration → Villas gains Deactivate/Reactivate and safe Remove (blocked with a specific reason whenever real data still references the villa); Configuration → Villa Mapping gains Edit and an "Unresolved" quick-fix banner alongside the existing Add/Remove — `DATA_MODEL.md` §1/§2. A Villa/Room-Villa-Mapping configuration change now automatically re-resolves already-imported reservations from already-stored data, with no re-upload — `IMPORT_LOGIC.md` §6. Summary's month range is corrected to January of the current year through 12 months ahead (dynamic), replacing "current month through year-end" — `REPORTING_LOGIC.md` §3. No new domain, no navigation-structure change, no financial-logic change.
