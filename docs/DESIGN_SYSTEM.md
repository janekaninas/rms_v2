...
Two related but distinct things, easy to conflate: the **app shell** (sidebar + page chrome) never scrolls away, and a **dense table page** (Monthly Performance's matrix, All Bookings' ledger — any future long/wide table) manages its own scrolling independently of the page around it. Applied to both pages identically; a third table page adopting this pattern should follow the same technique rather than inventing a variant.
...
- **`[NEW]` Full §3b treatment, not just pinned columns**: the page header (title + search/status filters) is a `shrink-0` zone that never scrolls, the table header row is sticky (`top: 0`), and the table lives in its own single `min-h-0 flex-1 overflow-auto` container (a raw `<table>`, not the shared `Table` wrapper, for the same nested-scroll-context reason as Monthly Performance) — this is the identical pattern §3b defines, applied to a second page. The four pinned-column header cells are corner cells (sticky in both directions) and use the corner z-index (30); other header cells use the header z-index (20); pinned body cells use the body z-index (10). Pagination sits in its own `shrink-0` zone below the table, alongside the header, never inside the scrolling region.
