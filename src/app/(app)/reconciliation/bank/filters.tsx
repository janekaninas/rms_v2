...
const STATUSES: BankReconciliationStatus[] = [
  "AWAITING_BANK",
  "MATCHED",
  "PARTIAL",
  "VARIANCE",
...
export function BankReconciliationFiltersBar({ bankAccounts, channels }: { bankAccounts: BankAccount[]; channels: Channel[] }) {
  ...
  return (
    <div className="mb-4 flex flex-wrap items-center gap-3">
      <Select value={searchParams.get("account") ?? "__all__"} onValueChange={(v) => setParam("account", v === "__all__" ? "" : v)}>
        <SelectTrigger className="w-56">
          <SelectValue placeholder="All bank accounts" />
        </SelectTrigger>
        <SelectContent>
          <SelectItem value="__all__">All bank accounts</SelectItem>
          {bankAccounts.map((a) => (
